...
## 2026-09-10 T-007: ファビコンの追加

### 実施内容
- index.htmlに`<link rel="icon">`が一つも定義されておらず、ブラウザタブが既定のアイコン（または`/favicon.ico`への404リクエスト、Playwright QA中に確認していたが実害のないノイズとして扱っていたもの）になっていることに気づいた。T-001のデザイン刷新の趣旨（「より洗練されたゲーム画面」）に対し、タブアイコンが未設定なのは細部の詰めが甘い状態だったため対応した。
- 既存の配色トークン（`--color-washi`背景・`--color-sumi`枠線・`--color-shu`文字）に合わせた円形のSVGファビコン（`favicon.svg`、常用漢字「熟」を配置）を新設し、`index.html`に`<link rel="icon" type="image/svg+xml" href="./favicon.svg">`を追加した。ビルド不要・追加バイナリ資産なし（インラインではなく別ファイルとして切り出し、通常のfaviconの慣習に合わせた）。

### 結果
- Playwrightで`favicon.svg`単体のレンダリング、および`index.html`から`<link rel="icon">`が正しく解決されリクエストが失敗していないことを確認した。
- 64x64のビューポートでスクリーンショットを撮り、円・枠線・文字が意図通り中央に収まっていることを目視確認した。
