...
## D-004: WCAGコントラスト比未達の`.result-card-rank`を、トークンではなく該当ルールのみ修正する

- 日付: 2026-09-10
- 状態: 採用

### 背景
- T-001のデザイン刷新で使用した配色トークンについて、正式なWCAGコントラスト比監査を行っていなかった。監査の結果、`.result-card-rank`（リザルト画面の称号表示）が`--color-ki`（#8b6e4e）を`--color-washi`（#f1ead9）背景に使っており、font-size 0.8rem（小さめ文字、AA基準4.5:1）に対し3.95:1しか達していないことが判明した。

### 決定
- `--color-ki`トークン自体は変更しない。`.result-card-rank`の`color`のみ、同系色でより暗い`#765e42`（同背景に対し5.08:1）に直接上書きする。

### 理由
- `--color-ki`は`.result-card-rank`以外に、`.subtitle`の装飾罫線（グラデーション）や`.debug-panel`の破線ボーダーでも使われている。これらは装飾要素でありWCAGのテキストコントラスト基準の対象外のため、トークン自体を暗くすると本来変更不要な箇所の見た目まで変わってしまう（判定ラダー: 既存トークンの流用よりも、影響範囲を必要最小限に抑えるスコープドな上書きを優先）。
- 色相（木・枯茶色系）を保ったまま明度のみ下げることで、デザインの一貫性を崩さずにAA基準を安全マージン込みで達成できることをコントラスト比計算で確認した。

### 影響
- `.result-card-rank`の文字色のみの変更。他のCSSルール・レイアウトへの影響はない。
- 修正後、実際にstyle.cssで使われている前景/背景の組み合わせ14件すべてがWCAG AA基準を満たすことを確認済み。
